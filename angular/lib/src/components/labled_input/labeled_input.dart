...
/// и область для вывода подсказок/сообщений об ошибках.
///
...
/// * [hint]::[String] - подсказка/сообщение об ошибке
///
...
class LabeledInput implements AfterViewInit, OnDestroy {
  /// Инжектор
  ///
  /// Используется для инжектирования эелемента управления формы [NgControl], если
  ...
  @Output()
  Stream<String> get onBlur => _blurController.stream;
  ...
  void ngAfterViewInit() {
    ...
    input
      ..onInput.listen((_) {
        // при вводе в `input` генерируем соответствующее событие
        _inputController.add(value);
      })
      ..onBlur.listen((_) {
        // при выходе из `input` генерируем соответствующее событие
        _blurController.add(null);
      });
  }

  /// Перед уничтожением компонента закрывает потоки событий [onBlur] и
  /// [onInput]
  @override
  void ngOnDestroy() {
